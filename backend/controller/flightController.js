...
export const getFlights = async (req, res) => {
  ...
  try {
    const fromRegex = from ? new RegExp(`^${from.trim()}$`, "i") : null;
    const toRegex = to ? new RegExp(`^${to.trim()}$`, "i") : null;

    const query = {};
    if (fromRegex) query.from = fromRegex;
    if (toRegex) query.to = toRegex;

    // Try finding exact date match first
    let flights = [];
    if (departDate) {
      flights = await Flight.find({ ...query, departDate });
    }

    // Fallback: search by route if no exact date match
    if (flights.length === 0) {
